...
import { createHutkoCheckout } from "@/lib/hutko";
import {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await createHutkoCheckout({
      orderId,
      orderDescription: summarizeOrder(items),
      amountUah,
      customerName: name,
      customerPhone: phone,
      customerEmail: email || undefined,
      delivery: `${delivery.serviceLabel}; ${city}; ${branch || postalCode}`,
      comment,
      reservationData: {
        orderId,
        delivery,
    ...
    if (order) {
      await saveOrder({
        ...
        paymentStatus: "create_checkout_failed"
      }).catch(() => undefined);
    }
