...
export function CheckoutForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/payments/hutko/create", {
        method: "POST",
      ...
      if (!response.ok) throw new Error(data.error || "Не удалось создать оплату Hutko.");
  ...
  return (
    <form className="order-panel" onSubmit={submit} id="checkout">
      <div className="section-head" style={{ marginBottom: 22 }}>
        <span className="kicker">Оплата только Hutko / PUMB</span>
      ...
      <div className="total-box">
        <div>
          <small>Итого к оплате через Hutko</small>
          <br />
      ...
      <button className="btn btn-primary" type="submit" disabled={loading} style={{ width: "100%" }}>
        {loading ? "Создаем платеж…" : "Оплатить через Hutko"}
      </button>
      ...
      <p className="disclaimer" style={{ marginTop: 14 }}>
        ...
        callback Hutko можно автоматически сформировать ТТН через API выбранной службы доставки.
      </p>
    </form>
  );
}
